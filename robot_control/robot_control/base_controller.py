...
class BaseController(threading.Thread):
    ...
    def loadModelAndPublishers(self, xacro_path=None):
        ...
        # ROS1 -> ROS2 name mapping: gazebo_ros2 (Gazebo Classic, humble) advertises the
        # simulation-control services at the ROOT namespace (no '/gazebo/' prefix).
        #   * /pause_physics, /unpause_physics (std_srvs/Empty) come from the
        #     gazebo_ros_init system plugin and are available out of the box.
        # COMPATIBILITY CAVEAT (needs runtime verification against `ros2 service list`):
        #   The model-state / physics-properties / body-wrench services below use the
        #   ROS1 gazebo_ros API surface. In gazebo_ros2 these have different names/types
        #   and require extra system plugins loaded in the .world file:
        #     * set_model_state  -> /set_entity_state (gazebo_msgs/SetEntityState) via the
        #       gazebo_ros_state plugin.
        #     * apply_body_wrench -> /apply_link_wrench (gazebo_msgs/ApplyLinkWrench) via the
        #       gazebo_ros_force_system plugin.
        #     * get/set_physics_properties have no direct gazebo_ros2 equivalent.
        #   _call_service() degrades gracefully (logs a warning and returns None) when a
        #   service is missing, so an un-migrated call disables that feature instead of
        #   crashing the controller. See go1_port_report.md ("remaining items").
