...
ROS2 (Humble) port of base_controllers/utils/common_functions.py
--------------------------------------------------------------------------
Translation notes (rospy -> rclpy / ament):
  * rospkg.RosPack().get_path(pkg)      -> ament_index_python get_package_share_directory(pkg)
  * ROS1 global param server (/robot_description, /flywheel*, ...) does not exist
    in ROS2, so the robot model is (re)generated deterministically by running the
    `xacro` CLI on <robot>_description/robots/<robot>.urdf.xacro and loaded into
    Pinocchio. This mirrors the ROS1 "offline" branch of getRobotModelFloating.
  * `rosrun xacro xacro`                -> `xacro` (ROS2 ships xacro as a plain CLI)
  * roscore / roslaunch python API helpers (checkRosMaster / launchFileNode /
    startNode / spawnModel) are replaced by `ros2 launch` / `ros2 run` subprocess
    equivalents; the Gazebo bring-up in this workspace is done by the
    ros_impedance_controller launch files, so these are rarely needed here.
  * tf2_ros.StaticTransformBroadcaster exists unchanged in ROS2.
ALL plotting helpers below `def subplot(...)` are pure matplotlib and copied
verbatim from the ROS1 source.
"""
import os
import subprocess
import sys
...
def _find_package_path(package_name):
    """Return the installed share path of a package (ROS2 ament index)."""
...
def _generated_urdf_dir():
    """Directory where generated urdfs are written (LOCOSIM_DIR if set, else /tmp)."""
    path = os.environ.get('LOCOSIM_DIR', None)
    if path is None:
        path = "/tmp/locosim"
    out = os.path.join(path, "robot_urdf", "generated_urdf")
    os.makedirs(out, exist_ok=True)
    return out


def loadXacro(package_name, model_name, additional_args=None):
    """Generate the URDF from a robot xacro and write it to the generated_urdf dir.
    Returns the path of the written urdf."""
    print(colored("Loading xacro for %s inside %s" % (model_name, package_name), "blue"))
    xacro_path = os.path.join(_find_package_path(package_name), 'robots', model_name + '.urdf.xacro')
    if not os.path.isfile(xacro_path):
        print(colored("Xacro file %s.urdf.xacro does not exist!" % model_name, "red"))
    urdf = _run_xacro(xacro_path, additional_args)
    urdf_location = os.path.join(_generated_urdf_dir(), model_name + ".urdf")
    with open(urdf_location, "w") as f:
        f.write(urdf)
    return urdf_location


def spawnModel(package_name, model_name='', spawn_pos=np.array([0., 0., 0.]), spawn_orient=np.array([0., 0., 0.])):
    """Spawn a model in Gazebo Classic using the ROS2 gazebo_ros spawn_entity node."""
    urdf_location = loadXacro(package_name, model_name)
    print(colored("Spawning %s" % model_name, "blue"))
    args = ["ros2", "run", "gazebo_ros", "spawn_entity.py",
            "-entity", model_name, "-file", urdf_location,
            "-x", str(spawn_pos[0]), "-y", str(spawn_pos[1]), "-z", str(spawn_pos[2]),
            "-R", str(spawn_orient[0]), "-P", str(spawn_orient[1]), "-Y", str(spawn_orient[2])]
    subprocess.Popen(args)


def launchFileGeneric(launch_file):
    """Start a ROS2 launch file in a subprocess (ROS1 roslaunch python API replacement)."""
...
def sendStaticTransform(parent, child, x_pos=np.zeros(3), quat=np.array([1, 0, 0, 0]), static_broadcaster=None):
    ...
    if static_broadcaster is None:
        if not rclpy.ok():
            rclpy.init()
        node = rclpy.create_node('static_tf_broadcaster')
    ...
    static_transformStamped.transform.translation.x = float(x_pos[0])
    static_transformStamped.transform.translation.y = float(x_pos[1])
    static_transformStamped.transform.translation.z = float(x_pos[2])
    static_transformStamped.transform.rotation.w = float(quat[0])
    static_transformStamped.transform.rotation.x = float(quat[1])
    static_transformStamped.transform.rotation.y = float(quat[2])
    static_transformStamped.transform.rotation.z = float(quat[3])
    static_broadcaster.sendTransform(static_transformStamped)


def getRobotModelFloating(robot_name="hyq", xacro_path=None, additional_urdf_args=None):
    """Build a floating-base Pinocchio model.

    ROS2: the URDF is (re)generated by running xacro on the robot description
    package and then loaded into Pinocchio with a free-flyer root joint. This is
    deterministic and does not depend on a global parameter server.
    """
    if xacro_path is None:
        xacro_path = os.path.join(_find_package_path(robot_name + '_description'),
                                  'robots', robot_name + '.urdf.xacro')
    urdf = _run_xacro(xacro_path, additional_urdf_args)
    urdf_location = os.path.join(_generated_urdf_dir(), robot_name + ".urdf")
    with open(urdf_location, "w") as f:
        f.write(urdf)
    print("URDF generated at " + urdf_location)
...
def getRobotModel(robot_name="hyq", generate_urdf=False, xacro_path=None, additional_urdf_args=None, floating_base=False):
    """Build a (fixed- or floating-base) Pinocchio model.

    ROS2: when generate_urdf is True the URDF is produced by running xacro on the
    robot description package; otherwise a pre-existing urdf/srdf pair under
    $LOCOSIM_DIR/robot_urdf is loaded (unchanged from ROS1).
    """
    ...
    if generate_urdf:
        if xacro_path is None:
            xacro_path = os.path.join(_find_package_path(robot_name + '_description'),
                                      'robots', robot_name + '.urdf.xacro')
        urdf = _run_xacro(xacro_path, [additional_urdf_args] if additional_urdf_args else None)
        urdf_location = os.path.join(_generated_urdf_dir(), robot_name + ".urdf")
        with open(urdf_location, "w") as f:
            f.write(urdf)
        print("URDF generated at " + urdf_location)
...
def plotJoint(name, time_log, q_log=None, q_des_log=None, qd_log=None, qd_des_log=None, qdd_log=None, qdd_des_log=None, tau_log=None, tau_ffwd_log = None, tau_des_log = None, joint_names = None, q_adm = None,
    ...
    # Default joint labels for common robot types
    labels_ur = ["1 - Shoulder Pan", "2 - Shoulder Lift", "3 - Elbow", "4 - Wrist 1", "5 - Wrist 2", "6 - Wrist 3"]
    labels_quadruped = ["LF_HAA", "LF_HFE", "LF_KFE", "RF_HAA", "RF_HFE", "RF_KFE", "LH_HAA", "LH_HFE", "LH_KFE", "RH_HAA", "RH_HFE", "RH_KFE"]
    labels_flywheel2 = ["joint_" + str(i) for i in range(14)]  # Generic fallback
    labels_flywheel4 = ["joint_" + str(i) for i in range(16)]  # Generic fallback
