"""
ROS2 (Humble) port of base_controllers/quadruped_controller.py
--------------------------------------------------------------------------
Translation notes (rospy -> rclpy). See base_controller.py for the shared
helpers (self.node, startExecutor, _call_service, _now, _sleep, _time_sec)
that this subclass inherits from BaseController:
  * rospy.Subscriber(topic, T, cb)  -> self.node.create_subscription(T, topic, cb, qos)
  * tf broadcaster sendTransform(...) -> self._broadcast_world_transform() (tf2_ros)
  * ros.Rate(1/dt)                  -> self.node.create_rate(1/dt)
  * ros.sleep(t)                    -> self._sleep(t)
  * ros.is_shutdown()               -> not rclpy.ok()
  * ros.ROSInterruptException / ros.service.ServiceException -> (KeyboardInterrupt, RuntimeError)
  * ros.signal_shutdown(...)        -> dropped (handled by deregister_node / rclpy.shutdown)
  * gazebo_ros.gazebo_interface.set_model_configuration_client(...)
    -> gazebo_msgs/srv/SetModelConfiguration service client (/gazebo/set_model_configuration)
  * PidManager(joint_names)         -> PidManager(self.node, joint_names)  (ROS2 PidManager
    needs the node handle)
All kinematics / dynamics / Pinocchio / WBC / IK math is unchanged.
"""
...
class QuadrupedController(BaseController):
    ...
    def initSubscribers(self):
        ...
        if self.real_robot:
            self.sub_imu_lin_acc = self.node.create_subscription(
                Vector3, "/" + self.robot_name + "/trunk_imu", self._receive_imu_acc_real, qos)
            self.sub_imu_euler = self.node.create_subscription(
                Vector3, "/" + self.robot_name + "/euler_imu", self._receive_euler, qos)
            self.sub_pose = self.node.create_subscription(
                Odometry, "/" + self.robot_name + "/ground_truth", self._receive_pose_real, qos)
        else:
            self.sub_imu_lin_acc = self.node.create_subscription(
                Imu, "/" + self.robot_name + "/trunk_imu", self._receive_imu_acc, qos)
        ...
        # if self.real_robot:
        #     self.sub_contact_force_z = self.node.create_subscription(
        #         Float64MultiArray, "/" + self.robot_name + "/contact_force_z", self._receive_contact_force_real, qos)

        # start spinning so callbacks fire (rospy did this implicitly)
        self.startExecutor()
    ...
    def _receive_pose(self, msg):
        ...
        # tf broadcast: world -> base_link (tf2_ros TransformStamped, see BaseController)
...
###W
    ...
    def startController(self, world_name=None, xacro_path=None, use_ground_truth_pose=True, use_ground_truth_contacts=True, additional_args=[]):
        ...
        self.rate = self.node.create_rate(1 / self.dt)
    ...
    def _set_model_configuration(self, joint_names, joint_positions):
        """ROS2 replacement for gazebo_ros.gazebo_interface.set_model_configuration_client.
        Calls the /gazebo/set_model_configuration service (gazebo_msgs/srv/SetModelConfiguration)
        to instantaneously set the robot joints to a given configuration in Gazebo."""
        if not hasattr(self, 'set_model_configuration_client'):
            self.set_model_configuration_client = self.node.create_client(
                SetModelConfiguration, '/gazebo/set_model_configuration')
        req = SetModelConfiguration.Request()
        req.model_name = self.robot_name
        req.urdf_param_name = ''
        req.joint_names = list(joint_names)
        req.joint_positions = list(map(float, joint_positions))
        return self._call_service(self.set_model_configuration_client, req)
    ...
    def reset(self, basePoseW=None, baseTwistW=None, resetPid=False):
        ...
        # ROS2 replacement for gazebo_ros.gazebo_interface.set_model_configuration_client(...)
        self._set_model_configuration(self.joint_names, self.qj_0)
