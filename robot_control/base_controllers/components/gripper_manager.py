...
# Date: 23-10-2022
#
# ROS2 (Humble) port of base_controllers/components/gripper_manager.py
# --------------------------------------------------------------------------
...
#   * ros.wait_for_service/ServiceProxy -> node.create_client + wait_for_service + call_async
#   * ros.sleep(t)                   -> time.sleep(t)
# The gripper kinematics mapping and the raw-socket URCap scripting for the real
# robot are pure logic and are kept unchanged.
import math
import time
import socket
...
class GripperManager():
    def __init__(self, gripper_type, real_robot_flag=False, dt=0.001, gripping_duration=5., node=None):
    ...
    def move_gripper_callback(self, req, response=None):
        ...
        # ROS2 service callbacks receive/return a response object
    ...
    def resend_robot_program(self):
        time.sleep(1.5)
        client = self.node.create_client(Trigger, "/ur5/ur_hardware_interface/resend_robot_program")
    ...
    def move_gripper(self, diameter=30, status='close'):
        ...
        # this is for the real robot, is a service call that sends a string directly to the URcap driver
        ...
        scripts_path = get_package_share_directory('ur_description') + '/gripper/scripts/'
